...
InfantryBasic::InfantryBasic(int m, int ws, int bs, int s, int t, int w, int i, int a, int ld, int as, Model::BaseType baseType) : _m(m), _ws(ws), _bs(bs), _s(s), _t(t), _w(w), _i(i), _a(a), _ld(ld), _as(as), _baseType(baseType)
{
}

int InfantryBasic::getMovement() {return _m;}
