...
public:
    ...
    int roll()
    {
        ...
        {
            if(DEBUG) if( i != 0) std::cerr << ",";
            int roll = 1 + std::rand() % getSize();
            if(DEBUG) std::cerr << roll;
            total += roll;
        }
        if(DEBUG) std::cerr << "}";
        return total;
...
class Weapon {
public:
    Weapon(std::string name, int s, std::shared_ptr<NumberSource> ap, std::shared_ptr<NumberSource> n, std::shared_ptr<NumberSource> d) : _name(name), _s(s), _ap(ap), _n(n), _d(d) {}
    std::string getName(){return _name;}
    int getStrength(){return _s;}
...
private:
    std::string _name;
    int _s;
    std::shared_ptr<NumberSource> _ap, _n, _d;
...
int getWoundsSuffered(Model shooter, Weapon w, Model shootee)
{
    ...
    if( w.getStrength() * 2 <= shootee.getToughness() ) 
        ...
        woundOn = 3;
    if(DEBUG) std::cerr << ",woundOn=" << woundOn << "+,numWounds=";
    int numWounds = Dice(numHits,6).rollGreaterEqual(woundOn);
    if(DEBUG) std::cerr << numWounds;
    //roll saves
    if(DEBUG) std::cerr << ",saveOn=" << shootee.getArmourSave() - w.getArmourPiercingValue() << "+,numSaved=";
    int numSaved = Dice(numWounds,6).rollGreaterEqual(shootee.getArmourSave() - w.getArmourPiercingValue());
    if(DEBUG) std::cerr << numSaved;
    //calculate wounds suffered
    if(DEBUG) std::cerr << ",woundsSuffered=";
...
int main()
{
    ...
    Weapon bolter("bolter", 4, C0, C1, C1);
    Weapon lascannon("lascannon", 9, C<-3>(), C1, D6);
    Weapon acid_spray("acidSpray", 7/*actually user*/, C<-1>(), D6, D3);
    acid_spray.attr(Attribute("autohit"));
    Weapon fleshborer_hive("fleshborerHive", 5, C0, C<20>(), C1);
    Weapon stinger_salvo("stingerSalvo", 5, C<-1>(), C<4>(), C1);
    Weapon rupture_cannon("ruptureCannon", 10, functionalSource([&rupture_cannon](){if(rupture_cannon.property("num_hits") == 2) return -4; return -1;}), C<2>(), functionalSource([&rupture_cannon](){if(rupture_cannon.property("num_hits") == 2) return Dice(1,6).roll(); return 2;}));
    Weapon fleshborer("fleshborer", 4, C0, C1, C1);
    Weapon devourer("devourer", 4, C0, C<3>(), C1);
    std::vector<Model> allModels = {
            Model("SpaceMarine", 6, 3, 3, 4, 4, 1, 1, 8, 3, {bolter, lascannon}),
            ...
            Model("TyranidWarrior", 6, 3, 4, 4, 4, 3, 3, 9, 4, {devourer}),
